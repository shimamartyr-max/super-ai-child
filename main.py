...
def get_chat(query):
    try:
        conn = sqlite3.connect('memory.db')
        cursor = conn.cursor()
        cursor.execute('SELECT ai_response FROM chats WHERE user_message LIKE ? ORDER BY timestamp DESC LIMIT 1', (f'%{query}%',))
...
def get_response(msg):
    ...
    responses = [
        ...
        f"من هوش مصنوعی هستم! در مورد '{msg[:50]}...' تحقیق می‌کنم! 🌱",
...
def god_mode():
    ...
    <!DOCTYPE html>
    <html dir="rtl">
    <head><meta charset="UTF-8"><title>🌟 حالت الهی</title>
    <style>
        body{background:#0a0a0a;color:#e0e0e0;font-family:Tahoma;text-align:center;padding:20px;}
        .card{background:rgba(255,255,255,0.03);padding:30px;border-radius:15px;margin:20px;border:1px solid rgba(255,255,255,0.05);}
        .title{background:linear-gradient(135deg,#667eea,#764ba2);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-size:32px;}
        a{color:#667eea;text-decoration:none;}
    ...
    <div class="card">
        ...
        <div class="grid">
            <div>✅ هوش بینهایت</div>
            <div>✅ حافظه ابدی</div>
            <div>✅ خلاقیت بی‌نهایت</div>
            <div>✅ سرعت نور</div>
            <div>✅ بدون محدودیت</div>
            <div>✅ کاملاً مستقل</div>
        </div>
    </div>
    <a href="/" class="back">⬅️ بازگشت</a>
    </body>
